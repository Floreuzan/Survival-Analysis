...
- "David G. Kleinbaum and Mitchel Klein, Survival Analysis. A Self-Learning Text, Third Edition.”
...
*   Survival object
    ...
    *   Transforming the survival curve
        ...
        2- “event”: plots **cumulative events**
        3- "cumhaz” plots the **cumulative hazard**
...
*   $e_{if}$ = expected counts in group i at time f (in our case: $e_1 = (\frac{n_{1f}}{n_{1f} + n_{2f}})(m_{1f} + m_{2f})$, $e_2 = (\frac{n_{2f}}{n_{1f} + n_{2f}})(m_{1f} + m_{2f})$)

*   $\#$ of failure times for the group i is: $O_{i} - E_{i} = \sum_{f=1}^{27} (m_{if} - e_{if})$
...
Therefore, the estimated variance of $O_2 - E_2$ is computed from the variance formula above to be 6.2685. The log–rank statistic then is obtained by squaring 10.26 and dividing by 6.285, which yields 16.793, as shown on the computer printout.
